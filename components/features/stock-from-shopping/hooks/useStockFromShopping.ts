...
export default function useStockFromShopping() {
  ...
  const handleExpiryChange = (id: number, value: string) => {
    setItems((prevItems) =>
      prevItems.map((item) => (item.id === id ? { ...item, expiry: value } : item))
    )
  }

  const handleCategoryChange = (id: number, value: string) => {
    setItems((prevItems) =>
      prevItems.map((item) => (item.id === id ? { ...item, category: value } : item))
    )
  }
  ...
  const addSelectedToStock = async () => {
    ...
    const { error } = await supabase.from('Foods').insert(itemsToInsert)
