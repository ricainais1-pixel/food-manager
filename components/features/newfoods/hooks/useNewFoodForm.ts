...
export default function useFoodForm() {
  ...
  const updateFood = async (id: number, field: string, value: string | number) => {
    ...
    const dbValue =
      field === 'count' ? (typeof value === 'string' ? parseInt(value) || 1 : value) : value
    ...
    setFoods(foods.map((food) => (food.id === id ? { ...food, [field]: dbValue } : food)))
  }
  ...
  const handleRegisterAll = async () => {
    ...
    const unsavedFoods = allFoods.filter(
      (f: NewFood) => f.id < 0 && f.name.trim() && f.expiry && f.category
    )
