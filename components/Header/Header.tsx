  ...
  return (
    <header className="sticky top-0 bg-lime-200 border-b z-50">
      <div className="flex items-center justify-between px-8 py-5">
        ...
        <nav className="hidden md:block">
          {session ? (
            <ul className="flex gap-6 text-lg font-medium">
              ...
              <li className="hover:text-amber-900 cursor-pointer">
                <span onClick={handleLogout} className="cursor-pointer hover:text-amber-900">
                  ログアウト
                </span>
              </li>
            </ul>
            ...
            <ul className="flex gap-6 text-lg font-medium">
              ...
              <li className="hover:text-amber-900 cursor-pointer">
                <Link
                  href="/register"
                  onClick={() => console.log('新規登録リンクがクリックされました')}
                >
                  新規登録
                </Link>
              </li>
        ...
        <Button onClick={toggleMenu} className="md:hidden text-2xl font-bold hover:text-amber-900">
          {isOpen ? '✕' : '☰'}
        </Button>
      </div>
        ...
        <div className="md:hidden bg-lime-300 border-t px-8 py-4">
          {session ? (
            <ul className="flex flex-col gap-4 text-lg font-medium">
              ...
              <li className="hover:text-amber-900 cursor-pointer">
                <Link href="/newfoods" onClick={() => setIsOpen(false)}>
                  登録
                </Link>
              </li>
              <li className="hover:text-amber-900 cursor-pointer">
                <Link href="/shopping" onClick={() => setIsOpen(false)}>
                  購入リスト
                </Link>
              </li>
              <li className="hover:text-amber-900 cursor-pointer">
